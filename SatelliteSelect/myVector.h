...
public:
	CVector		();
	CVector		(tagVECTOR vIn);
	~CVector	() = default;
public:
	tagVECTOR 	GetVector	();
	void 	SetVector	(tagVECTOR vIn);
	int		GetDepth	();
	void 	SetDepth	(int iDepth);
 


	double 	Dot		( tagVECTOR vIn );
	tagVECTOR	Cross	( tagVECTOR vIn );
	double 	Mag		( );
    tagVECTOR 	Unit	();
	tagVECTOR	Diff	( tagVECTOR vIn );
	tagVECTOR 	Sum		( tagVECTOR vIn );
	tagVECTOR 	Scale	( double u );
